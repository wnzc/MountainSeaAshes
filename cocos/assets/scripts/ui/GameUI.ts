...
export class GameUI {
  ...
  private preloadArt(): void {
    ...
    paths.add('textures/ui/tower_base');
    paths.add('textures/ui/card_frame_0');
    paths.add('textures/ui/card_frame_1');
    paths.add('textures/ui/card_frame_2');
    paths.add('textures/ui/card_frame_3');
    paths.add('textures/ui/card_frame_4');
    paths.add('textures/maps/mirror_stream_bg_v2');
  ...
  private makeButton(parent: Node, text: string, w: number, h: number, onDown: () => void): Button {
    const n = new Node('Btn_' + text);
    ensureTransform(n, w, h);
    parent.addChild(n);
    const g = n.addComponent(Graphics);
    drawRect(g, -w / 2, -h / 2, w, h, new Color(48, 54, 48, 230));
    drawRect(g, -w / 2 + 3, -h / 2 + 3, w - 6, h - 6, new Color(70, 62, 36, 200));
    this.makeLabel(n, text, 28, hexColor('#F3EFE4'), w, h);
  ...
  private buildHud(): void {
    ...
    ensureTransform(hud, this.mapW, 140);
    hud.setPosition(0, this.mapH / 2 - 70);
    this.uiLayer.addChild(hud);

    this.hudWave = this.makeLabel(hud, '1/8', 32, hexColor('#FAF3E4'), 180, 56);
    this.hudWave.node.setPosition(-380, 0);
    this.hudHp = this.makeLabel(hud, '❤ 20', 32, hexColor('#FAF3E4'), 160, 56);
    this.hudHp.node.setPosition(-180, 0);
    this.hudGold = this.makeLabel(hud, '◉ 320', 32, hexColor('#FAF3E4'), 180, 56);
    this.hudGold.node.setPosition(20, 0);

    const speedBtn = this.makeButton(hud, '×1', 110, 72, () => {
    ...
    speedBtn.node.setPosition(240, 0);

    const pauseBtn = this.makeButton(hud, 'Ⅱ', 110, 72, () => {
      this.battle.paused = true;
      this.pausePanel.active = true;
    });
    pauseBtn.node.setPosition(380, 0);
    ...
    this.toast = this.makeLabel(this.uiLayer, '', 28, hexColor('#FAF3E4'), 600, 56);
    this.toast.node.setPosition(0, this.mapH / 2 - 220);
    this.toast.node.active = false;
  }

  private buildCards(): void {
    const bar = new Node('CardBar');
    ensureTransform(bar, this.mapW, 180);
    bar.setPosition(0, -this.mapH / 2 + 100);
    ...
    for (const id of SPIRIT_ORDER) {
      ...
      ensureTransform(card, cw, 200);
      card.setPosition(x0, 0);
      x0 += cw + gap;
      bar.addChild(card);
      const g = card.addComponent(Graphics);
      drawRect(g, -cw / 2, -100, cw, 200, new Color(28, 34, 28, 240));
      drawRect(g, -cw / 2 + 4, -96, cw - 8, 192, new Color(40, 48, 40, 255));
      // 立绘
      const port = makeSpriteNode('Portrait', 110, cfg.sprite, card);
      ...
      const costLab = this.makeLabel(card, String(cfg.cost), 24, hexColor('#D4A84B'), cw, 32);
      costLab.node.setPosition(0, -78);
  ...
  private buildPanels(): void {
    // 塔位操作
    this.slotPanel = new Node('SlotPanel');
    ensureTransform(this.slotPanel, 980, 280);
    this.slotPanel.setPosition(0, -this.mapH / 2 + 360);
    this.slotPanel.active = false;
    this.uiLayer.addChild(this.slotPanel);
    const g = this.slotPanel.addComponent(Graphics);
    drawRect(g, -490, -140, 980, 280, new Color(18, 22, 18, 240));
    this.slotTitle = this.makeLabel(this.slotPanel, '', 36, hexColor('#FAF3E4'), 900, 50);
    this.slotTitle.node.setPosition(0, 90);
    this.slotMeta = this.makeLabel(this.slotPanel, '', 24, hexColor('#B8B0A0'), 900, 80);
    this.slotMeta.node.setPosition(0, 20);
    this.btnUpgrade = this.makeButton(this.slotPanel, '升级', 360, 80, () => {
    ...
    this.btnUpgrade.node.setPosition(-200, -80);
    this.btnSell = this.makeButton(this.slotPanel, '撤回', 360, 80, () => {
      if (this.battle.selectedSlot) {
    ...
    this.btnSell.node.setPosition(200, -80);

    // 开始
    this.startPanel = this.makeOverlay('开始');
    const startBtn = this.makeButton(this.startPanel, '进入山河', 420, 100, () => {
    ...
    // 暂停
    this.pausePanel = this.makeOverlay('暂停');
    ...
    // 结算
    this.resultPanel = this.makeOverlay('山河复色');
    this.resultTitle = this.resultPanel.getChildByName('Title')!.getComponent(Label)!;
    this.resultStats = this.makeLabel(this.resultPanel, '', 24, hexColor('#B8B0A0'), 700, 50);
    ...
    }).node.setPosition(0, -170);
    this.resultPanel.active = false;
  }

  private makeOverlay(title: string): Node {
    const panel = new Node('Panel_' + title);
    ensureTransform(panel, 800, 560);
    this.uiLayer.addChild(panel);
    const g = panel.addComponent(Graphics);
    drawRect(g, -400, -280, 800, 560, new Color(12, 16, 12, 230));
  ...
  updateView(dt: number): void {
    ...
    if (this.toastLife > 0) {
      this.toastLife -= dt;
      if (this.toastLife <= 0) this.toast.node.active = false;
    }
    // 清 fx
